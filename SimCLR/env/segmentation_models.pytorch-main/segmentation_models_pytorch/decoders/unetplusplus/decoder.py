...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        if attention_type == "ACmix":
            self.attention2 = md.Attention(attention_type, in_channels=out_channels, out_channels=out_channels)
            self.attention1 = md.Attention(
                attention_type, in_channels=in_channels + skip_channels, out_channels=in_channels + skip_channels
            )
        elif attention_type == "Axial":
            self.attention2 = md.Attention(attention_type, dim=out_channels)
            self.attention1 = md.Attention(
                attention_type, dim=in_channels + skip_channels
            )
        elif attention_type == "CrissCrossAttention":
            self.attention2 = md.Attention(attention_type, in_dim=out_channels)
            self.attention1 = md.Attention(
                attention_type, in_dim=in_channels + skip_channels
            )
        elif attention_type == "CoordAttention":
            self.attention2 = md.Attention(attention_type, inp=out_channels, oup=out_channels) 
            self.attention1 = md.Attention(
                attention_type, inp=in_channels + skip_channels, oup=in_channels + skip_channels
            )
        elif attention_type == "SimAM":
            self.attention2 = md.Attention(attention_type, channels=out_channels) 
            self.attention1 = md.Attention(
                attention_type, channels=in_channels + skip_channels
            )
